...
#   Write-PhaseTimingReport, Export-PhaseTimingTree, ...) are the 2-level compat
#   shims exported by Common.PowerShell (imported by Install-ModuleDependencies
#   above). Declare the three stages in run order so the tree lists each one -
#   even a stage that never ran because an earlier one failed. The stages below
#   run inside Invoke-WithPhaseTimer wrappers; the outer try/finally exports the
#   tree on the TIMING_TREE_OUTPUT_PATH opt-in so a parent orchestrator can graft
#   this run's timings under the part that shelled out to create-users.ps1.
# ---------------------------------------------------------------------------
...
try {
    ...
    Invoke-WithPhaseTimer -Name 'Read configs + resolve router IP' -Action {
        ...
        #    creds). All other provisioner fields (cpuCount, ramGB, etc.) are
        ...
        # values this and the following phases publish this way are read by
        # later phases, so they must land in the script scope, not the action's.
        ...
        #    Workloads in the per-environment private switch are not reachable
        #    from the host directly - the router VM forwards their SSH via
        #    MASQUERADE. Find any router row in the same VmProvisionerConfig
        #    batch (kind == 'router'), discover its upstream IP via Hyper-V
        #    KVP, and stamp it as _RouterVm on each workload so
        #    New-VmSshClientWithJump picks the jump-through-router path for that
        #    workload's session.
        #
        #    When no router row is present the batch predates feature 53 (or the
        #    operator deliberately runs a single-switch topology); every
        #    workload keeps the legacy direct-connect path and the join below
        #    proceeds as before.
        # -------------------------------------------------------------------

        # Import Hyper-V here (not in Install-ModuleDependencies) because Get-VM
        # / Get-VMNetworkAdapter are needed only when a router row is present,
        # and Install-ModuleDependencies runs unconditionally - importing it
        # there would fail on Hyper-V-absent hosts that the no-router path
        # supports.
        ...
        #    ConvertFrom-VmUsersConfigJson validates structure and emits each
        #    entry to the pipeline; @() collects all of them.
        # -------------------------------------------------------------------
    ...
    Invoke-WithPhaseTimer -Name 'Match + SSH-probe targets' -Action {
        ...
        # -------------------------------------------------------------------
        # 3. Join by vmName
        #    Build a hashtable index from the provisioner list for O(1) lookup,
        #    then match each users entry. Unmatched entries are warned and
        #    skipped - they likely reference a VM not yet provisioned or contain
        #    a typo in vmName.
        # -------------------------------------------------------------------
        ...
        # provisioner counterpart so downstream phases have both in one place.
        ...
        #    superset of an ICMP ping, since a successful TCP connect implies
        #    the host is up AND has bound port 22. Eliminates the post-reboot
        #    race where ICMP succeeds before sshd binds.
        #
    ...
    Invoke-WithPhaseTimer -Name 'Per-VM SSH reconcile' -Action {

        # -------------------------------------------------------------------
        # 5. Reconciliation via SSH
        #    For each reachable VM, open one SSH session and reconcile groups,
        #    users, and sudoers rules within it. The session is always closed in
        #    the finally block, even if an operation throws.
        #
        #    Security note: -AcceptKey auto-trusts the host key on first
        #    connection without verifying a fingerprint. This is acceptable on
        #    an internal Hyper-V network with statically provisioned IPs. Do NOT
        #    use -AcceptKey against untrusted or shared networks.
        #
        #    Sudo assumption: cloud-init adds the provisioned admin user to the
        #    sudo group with NOPASSWD via /etc/sudoers.d/90-cloud-init-users
        #    (Ubuntu default). If your image disables passwordless sudo, add
        #    -sudo-password handling before the useradd/usermod calls.
        # -------------------------------------------------------------------
        ...
        foreach ($t in $reachable) {
            ...
            #     Renci.SshNet.ForwardedPortLocal to the workload's :22, then
            #     connects through the loopback endpoint. The returned session's
            #     Dispose tears the tunnel down in the right order.
    ...
    # runner) sets TIMING_TREE_OUTPUT_PATH, serialise the phase tree to that
    # path so the parent can graft this run's timings under the part that
    # shelled out to create-users.ps1. Neutral variable name - the script does
    # not know who consumes it. Fires on success AND failure; the shim no-ops
    # when timings were never initialised. Unset = unchanged (no file written).
