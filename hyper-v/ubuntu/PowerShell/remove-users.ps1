...
#   this run's timings under the part that shelled out to remove-users.ps1.
# ---------------------------------------------------------------------------
...
try {
    ...
    Invoke-WithPhaseTimer -Name 'Per-VM SSH removal' -Action {

        # -------------------------------------------------------------------
        # 5. Removal via SSH
        #    For each reachable VM, open one SSH session and remove sudoers
        #    files, users, and declared groups within it. The session is always
        #    closed in the finally block, even if an operation throws.
        #
    ...
    # shelled out to remove-users.ps1. Neutral variable name - the script does
